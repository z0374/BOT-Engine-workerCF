...
async function handleUser(request, env) {
  ...
  try {
    ...
    let userState = await loadUserState(env, userId); // Carrega o estado atual do usuário

    // Se não existir, cria estado padrão
    if (!userState) {
      userState = {
        proces: '',
        state: '',
    ...
    if (userState.proces === '') {
      userState.proces = 'register_masterUser';
      userState.state = 'register_credentials_master_user';
    }

    // Processo de registro do usuário master
    if (userState.proces == 'register_masterUser') {
      
      switch (userState.state.toUpperCase()) {

        case 'REGISTER_CREDENTIALS_MASTER_USER':
          userState.procesCont = 0;
          userState.state = 'REGISTER_PIN_USER';
        ...
        case 'REGISTER_PIN_USER':
          ...
          userState.procesCont = 0;
          // Hashing seguro com PBKDF2 (armazena salt$hash em userState.texto)
          userState.texto = await hashPassword(messageText, PBKDF2_ITERATIONS); 
          userState.state = 'register_confirm_user';
          await saveUserState(env, userId, userState);
        ...
        case 'REGISTER_CONFIRM_USER':
          ...
          const isValidPin = await verifyPassword(messageText, userState.texto, PBKDF2_ITERATIONS);
          ...
          const delPUK = (await sendCallBackMessage(`⚠️ ATENÇÃO: Seu PUK é -| ${puk} |-.\nNÃO PERCA nem COMPARTILHE este código. Ele será necessário para redefinir ou desbloquear seu PIN.\nESTA MENSAGEM SERÁ APAGADA EM 15 segundos!`, chatId, env)).result.message_id;
            ...
            try {
              ...
              const result = await dataSave([userState.texto, userState.titulo, 'MASTER'], ['users', 'userData, chatId, tipo'], env, chatId);
          ...
          await saveUserState(env, userId, null); // Limpa estado do usuário
...
async function recUser(update, env) {
    ...
    try {
      ...
      if (result.results.length === 0) {
        ...
        await saveUserState(env, userId, {proces: 'register_masterUser', state: 'REGISTER_CREDENTIALS_MASTER_USER'}) 
